...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const navLinksVendas = document.querySelectorAll('.sidebar-nav a'); // Tipado como HTMLAnchorElement
    const sectionsVendas = document.querySelectorAll('.dashboard-page-content > .dashboard-section'); // Tipado como HTMLElement
    ...
    function updateActiveLinkAndTitleVendas(activeLink) {
        var _a;
        console.log("Vendas.ts: updateActiveLinkAndTitleVendas chamado com link:", activeLink === null || activeLink === void 0 ? void 0 : activeLink.href);
        ...
        if (activeLink) {
            ...
            if (tituloSecaoHeaderVendas) {
                ...
                let titulo = activeLink.dataset.title || ((_a = activeLink.textContent) === null || _a === void 0 ? void 0 : _a.trim()) || 'Dashboard';
        ...
        sectionsVendas.forEach(section => {
            ...
            if (sectionEl.id === targetId) {
                ...
                if (targetId === 'secao-dashboard' && dadosCompletosVendas.length > 0) {
                    ...
                    renderizarTabelaVendas(dadosCompletosVendas.filter(linha => filtrarLinhaVendas(linha, (filtroGeralInputVendas === null || filtroGeralInputVendas === void 0 ? void 0 : filtroGeralInputVendas.value) || '')));
                }
    ...
    function handlePageLoadAndNavigationVendas() {
        ...
        if (activeLinkElement) {
            ...
            const dashboardLinkFallback = document.querySelector('.sidebar-nav a[href="#dashboard"]');
    ...
    const calcularKPIsEVisualizacoesVendas = (dados) => {
            ...
            if (!isNaN(valorVendaNum)) {
                ...
                if (dataStr) {
                    ...
                    if (/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(dataStr)) {
                        ...
                        dataObj = new Date(Number(p[2]), Number(p[1]) - 1, Number(p[0]));
                    }
                    else if (/^\d{4}-\d{1,2}-\d{1,2}$/.test(dataStr)) {
                        const p = dataStr.split('-');
                        dataObj = new Date(Number(p[0]), Number(p[1]) - 1, Number(p[2]));
                    }
                    if (dataObj && !isNaN(dataObj.getTime())) {
                        const m = dataObj.getMonth() + 1;
                        const a = dataObj.getFullYear();
                        const k = `${a}-${m.toString().padStart(2, '0')}`;
                        if (!vendasPorMes[k])
                            vendasPorMes[k] = { total: 0, ano: a, mes: m };
        ...
        if (ctxTendenciaCanvas) { /* ... seu código de gráfico de tendência ... */
            ...
            if (ctx) {
                ...
                graficoTendenciaInstanceVendas = new Chart(ctx, { type: 'line', data: { labels, datasets: [{ label: 'Tendência de Vendas Mensais', data: valores, borderColor: corLinhaTendencia, backgroundColor: corAreaTendencia, tension: 0.3, fill: true, pointBackgroundColor: corLinhaTendencia, pointBorderColor: corTextoPrincipalDark, pointHoverBackgroundColor: corTextoPrincipalDark, pointHoverBorderColor: corLinhaTendencia }] }, options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, ticks: { callback: (v) => formatarMoedaVendas(v), color: corTextoSecundarioDark }, grid: { color: corBordasDark, drawBorder: false } }, x: { ticks: { color: corTextoSecundarioDark, maxRotation: 0, autoSkipPadding: 20 }, grid: { color: corBordasDark, display: false } } }, plugins: { legend: { display: true, labels: { color: corTextoSecundarioDark, padding: 15, font: { size: 11 } } }, tooltip: { bodyColor: corTextoPrincipalDark, titleColor: corTextoPrincipalDark, backgroundColor: corFundoCardsDark, borderColor: corBordasDark, borderWidth: 1, padding: 10, callbacks: { label: (c) => `${c.dataset.label || 'Vendas'}: ${formatarMoedaVendas(c.raw)}` } } } } });
            }
        }
    };
    ...
    const carregarDadosVendas = () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        if (!URL_PLANILHA_CSV_VENDAS || URL_PLANILHA_CSV_VENDAS.includes('COLE_AQUI') || URL_PLANILHA_CSV_VENDAS.length < 50) {
            ...
            if (loadingMessageDivVendas)
                loadingMessageDivVendas.style.display = 'none'; // Esconde loading em caso de erro de URL
            return;
        }
        try {
            const resposta = yield fetch(URL_PLANILHA_CSV_VENDAS);
            if (!resposta.ok) {
                throw new Error(`Falha ao buscar CSV de Vendas: ${resposta.status} ${resposta.statusText}. Verifique URL e permissões.`);
            }
            const textoCsv = yield resposta.text();
            if (!textoCsv || textoCsv.trim() === '') {
                throw new Error('CSV de Vendas vazio ou inválido.');
            }
            ...
            if (dadosCompletosVendas.length === 0 && ((_a = document.getElementById('secao-dashboard')) === null || _a === void 0 ? void 0 : _a.style.display) !== 'none') {
            ...
            if (loadingMessageDivVendas)
                loadingMessageDivVendas.style.display = 'none'; // Garante que loading desapareça em erro
    ...
    if (filtroGeralInputVendas) {
        filtroGeralInputVendas.addEventListener('input', (e) => {
            ...
            if (((_a = document.getElementById('secao-dashboard')) === null || _a === void 0 ? void 0 : _a.style.display) !== 'none') { // Só filtra se a seção do dashboard estiver visível
