...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const sectionsVendas = document.querySelectorAll('.dashboard-page-content > .dashboard-section'); // Ajustado para pegar seções filhas diretas
    const tituloSecaoHeaderVendas = document.getElementById('dashboard-titulo-secao'); // Supondo que você tenha este ID no seu HTML do header
        ...
        sectionsVendas.forEach(section => {
            ...
            if (sectionEl.id === targetId) {
                ...
                sectionEl.querySelectorAll('.kpi-card, .grafico-card, .card-secao, .secao-tabela-detalhada').forEach((card, index) => {
                    card.style.animation = 'none'; // Reseta animação
                    ...
                    options: {
                        ...
                        plugins: {
                            ...
                            tooltip: {
                                ...
                                callbacks: { label: (context) => `${context.label}: ${formatarMoedaVendas(context.raw)}` }
                            }
                        }
                    }
                });
    ...
    const renderizarTabelaVendas = (dadosParaRenderizar) => {
        ...
        if (noDataMessageDivVendas)
            noDataMessageDivVendas.style.display = 'none'; // Esconde mensagem de "sem dados"
    ...
    const filtrarLinhaVendas = (linha, termoBusca) => {
        if (!termoBusca)
            return true; // Se não há termo de busca, não filtra (mostra tudo)
    ...
    const carregarDadosVendas = () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            ...
            if (errorMessageDivVendas)
                errorMessageDivVendas.style.display = 'none'; // Esconde msg de erro se carregou bem
